 ...
 * Basic constants
 */
#define CSCHED_JINHO_TESTMS         5

#define CSCHED_DEFAULT_WEIGHT       256
#define CSCHED_TICKS_PER_TSLICE     3
#define CSCHED_TICKS_PER_ACCT       3
#define CSCHED_MSECS_PER_TICK       100
    ...
    const struct scheduler *ops, s_time_t now, bool_t tasklet_work_scheduled)
{
    ...
    // jinho logging
    //printk("Scheduled (%d:%d:%d)\n", cpu, snext->vcpu->domain->domain_id, snext->vcpu->vcpu_id);

    /*
     * Return task to run next...
     */
    ret.time = (is_idle_vcpu(snext->vcpu) ? -1 : MILLISECS(CSCHED_MSECS_PER_TSLICE));
    //ret.time = (is_idle_vcpu(snext->vcpu) ? -1 : MILLISECS(CSCHED_JINHO_TESTMS)); 
    ret.task = snext->vcpu;
