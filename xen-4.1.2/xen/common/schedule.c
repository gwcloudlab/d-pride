...
service_type_table svc_type[] = {
{5901, SCHED_SVC_ONLINE_ACTIVE},
{5902, SCHED_SVC_ONLINE_ACTIVE}
#define SERVICE_TYPE_MAX    2
};

/* opt_sched: scheduler - default to vdi: jinho modified */
static char __initdata opt_sched[10] = "vdi";
//static char __initdata opt_sched[10] = "credit";
//static char __initdata opt_sched[10] = "sedf";
...
extern const struct scheduler sched_vdi_def;
...
static uint32_t get_service_type(uint32_t source_port) 
{
    int i;
    for(i = 0; i < SERVICE_TYPE_MAX; i++) {
        if(svc_type[i].port == source_port) {
            return svc_type[i].type;
        }
    }
    return SCHED_SVC_UNKNOWN;
}

static long do_store_service_info(uint32_t domid, uint32_t source_port)
{
    struct domain *pdom;
    uint32_t service_type = get_service_type(source_port);
    pdom = get_domain_by_id((domid_t)domid);

    domain_lock(pdom);

    if(service_type != SCHED_SVC_UNKNOWN) {
        pdom->service_type = service_type;
        pdom->st_time = SCHED_SVC_TYPE_TIME_MAX;
    } else {
        // TODO: I have to justify this
        pdom->service_type = SCHED_SVC_DEFAULT;
        pdom->st_time = SCHED_SVC_TYPE_TIME_MAX;
    }
...
ret_t do_sched_op(int cmd, XEN_GUEST_HANDLE(void) arg)
{
    ...
    // jinho added
    case SCHEDOP_service:
    {
        struct sched_service sched_service;

        ret = -EFAULT;
        if ( copy_from_guest(&sched_service, arg, 1) )
            break;

        // confirmed that it is delivered well
        //printk("domid: %d, source: %d\n", sched_service.domid, sched_service.source);

        ret = do_store_service_info(
                sched_service.domid, sched_service.source);
        break;
    }
...
static void schedule(void)
{
    ...
    static s_time_t     max_time = 0; 
    static s_time_t     min_time = 0; 
    static s_time_t     acc_time = 0; 
    s_time_t     prev_time = 0; 
    ...
    // jinho added
    //printk("do_schedule() overhead: %lld - %lld = %lld\n", NOW(), prev_time, (NOW() - prev_time));
    if(acc_time > 1000000000) { // test and adjust
        printk("sched_stat [total:count:avg:max:min] = [%lld:%d:%lld:%lld:%lld]\n", 
                acc_time, count, acc_time/count, max_time, min_time);
        ...
        s_time_t temp_time = NOW() - prev_time; 

        // update
        acc_time += NOW() - prev_time;
