...
struct vsched_util util_param[SCHED_SVC_NUM] = {
    {1, 3000, 2000}, {2, 5000, 1600}, {3, 8000, 1300}, {4, 10000, 1000}
};
uint32_t tmax = 2000;
uint32_t duration = 1000;
uint32_t partial = 50; // 2%
...
void displayQueue(unsigned int cpu)
{
    ...
    {
        struct vsched_vcpu *iter_svc = __runq_elem(iter);
        printk("(%d:%d:%d)", cpu, iter_svc->vcpu->domain->domain_id, iter_svc->vcpu->vcpu_id);
        temp=1;
    }
    if(temp) printk("\n");
}
...
struct vsched_vcpu* updateAndPick(const int cpu, const struct scheduler *ops, struct vsched_vcpu *curr_svc) 
{
    ...
    //struct vsched_dom *sdom = curr_svc->sdom;
    struct domain *sdom = curr_svc->vcpu->domain;
    ...
    if(curr_svc->vcpu != NULL) {
        jprintk("------- utility update start(%d:%d:%d:%lld - %lld = %lld us)\n", 
            cpu, sdom->domain_id, curr_svc->vcpu->vcpu_id, 
            now, curr_svc->sched_time, sched_time);
    } else {
        jprintk("------- utility update start(%d:%lld us)\n", cpu, sched_time);
    }

    // Current One: The first one possibly dummy
    if(sdom->domain_id != DUMMY_DOM) {
        svc->vcpu->avgtime = svc->vcpu->avgtime     
                            + sched_time
                            - svc->vcpu->avgtime/partial;
        svc->vcpu->avgtime = (svc->vcpu->avgtime < 0)? 0:svc->vcpu->avgtime;
        svc->vcpu->utility = getUtil(svc);
        temp_util = svc->vcpu->utility;

        sdom->st_time -= sched_time;
        if(sdom->st_time <= 0) {
            sdom->st_time = SCHED_SVC_TYPE_TIME_MAX;
            if(sdom->service_type < (SCHED_SVC_NUM - 1)) {
                sdom->service_type += 1;
            } else {
                sdom->service_type = SCHED_SVC_OFFLINE_INACTIVE;
            }
        }
    }
    ...
    list_for_each( iter, runq )
    {
        ...
        //if(svc->vcpu == NULL || svc->vcpu->domain == NULL) continue;
        sdom = svc->vcpu->domain;

        // in-queue-check: if it is not in queue, should be changed
        if(curr_svc == svc) { in_q_flag = 1; }

        // let's not schedule this dummy: 
        // real scheduler also schedules this dummy
        if( sdom->domain_id == DUMMY_DOM ) continue;

        // not consider the same one
        if(curr_svc != svc) {
            // Calculate utility: only different ones come here 
            svc->vcpu->avgtime = svc->vcpu->avgtime     
                                - svc->vcpu->avgtime/partial;

            svc->vcpu->delay = now - svc->sched_time;
            ...
            if(curr_svc->vcpu->domain->domain_id == DUMMY_DOM ||
                    svc->vcpu->utility >= temp_util) {
                top = svc;
                temp_util = svc->vcpu->utility;
            }

            jprintk("(%d:%d:%d:%d:%lld:%d)", 
                cpu, sdom->domain_id, svc->vcpu->vcpu_id, 
                sdom->service_type, svc->vcpu->avgtime, 
                svc->vcpu->utility);
        } 

        // TODO: if there is only one VCPU except DUMMY, 
        // we don't need to increase the time
    }
    ...
    /*
    // === Old version without thinking about CPU division
    ...
    list_for_each_safe( iter_sdom, next_sdom, &prv->active_sdom )
    {
        ...
        list_for_each_safe( iter_vcpu, next_vcpu, &sdom->active_vcpu )
        {
            ...
            if(curr_svc == svc) { // scheduled one
                ...
                // TODO : service time for service management
                sdom->dom->st_time -= sched_time;
                if(sdom->dom->st_time <= 0) {
                    sdom->dom->st_time = SCHED_SVC_TYPE_TIME_MAX;
                    if(sdom->dom->service_type < (SCHED_SVC_NUM - 1)) {
                        sdom->dom->service_type += 1;
                    } else {
                        sdom->dom->service_type = SCHED_SVC_OFFLINE_INACTIVE;
                    }
                }
            }
            svc->vcpu->avgtime = (svc->vcpu->avgtime < 0)? 0:svc->vcpu->avgtime;
            ...
            // TODO: Equality should hold?
            if(svc->vcpu->utility > temp_util) {
                top = svc;
                temp_util = svc->vcpu->utility;
            }

            jprintk("(%d:%d:%d:%lld:%d)", 
                sdom->dom->domain_id, svc->vcpu->vcpu_id, 
                sdom->dom->service_type, svc->vcpu->avgtime, 
                svc->vcpu->utility);
        }
    }
    ...
    */
    ...
    if( top != NULL && top->vcpu != NULL )  {
        if( top->vcpu->domain->domain_id < VSCHED_STAT_MAX_DOM )  {
            vstat.dom[top->vcpu->domain->domain_id]++;
        }
        jprintk("Picked (%d:%d:%d:%d:%lld:%d)\n", cpu, 
            top->vcpu->domain->domain_id, top->vcpu->vcpu_id, 
            top->vcpu->domain->service_type, top->vcpu->avgtime, 
            top->vcpu->utility);
    }
    jprintk("Stat info : ");
    for(i = 0; i < VSCHED_STAT_MAX_DOM; i++) {
        jprintk("%d:", vstat.dom[i]);
    }
    jprintk("\n");

    //jprintk("------- utility update end..picked [%d:%d] \n", 
    //            top->vcpu->domain->domain_id, top->vcpu->vcpu_id);

    jprintk("utility update end ------- \n\n");
    ...
    const struct scheduler *ops, s_time_t now, bool_t tasklet_work_scheduled)
{
    ...
    /*
    printk("FAKE VCPU(%d:%d:%d) idle? %s\n", 
            cpu, fake_vcpu[cpu]->vcpu->domain->domain_id, fake_vcpu[cpu]->vcpu->vcpu_id,
            (is_idle_vcpu(fake_vcpu[cpu]->vcpu))? "YES":"NO");
    ...
    */
    ...
    // jinho modified: intercept next schedule
    // TODO: this should be here due to start_time
    //if(fake_vcpu[cpu] == NULL) fake_vcpu[cpu] = scurr; 
    //fake_vcpu[cpu] = updateAndPick(cpu, ops, fake_vcpu[cpu]); // all the utilites will be stored in vcpu
    // picking new vcpu does not work here.. sort? 

    // test run
    //util_next = updateAndPick(cpu, ops, scurr); // all the utilites will be stored in vcpu
    //snext = __runq_elem(&(util_next->runq_elem));

    //snext = __runq_get(cpu, scurr); // can this be NULL?? -> didn't work
    //snext = __runq_elem(&(util_next->active_vcpu_elem)); //TODO: check this out --> didn't work

    // Surprisingly, real scheduler schedules dymmy(32767)
    //snext = pinSched(cpu, scurr);
    ...
    */
    ...
    /*
    printk("%d:%d:0x%x = %d:%d:0x%x\n", 
        util_next->vcpu->domain->domain_id, util_next->vcpu->vcpu_id, (unsigned int)util_next, 
        snext->vcpu->domain->domain_id, snext->vcpu->vcpu_id, (unsigned int)snext);
    */

    //BUG();

    /*
    printk("REAL VCPU(%d:%d:%d) idle? %s\n", 
            cpu, snext->vcpu->domain->domain_id, snext->vcpu->vcpu_id,
            (is_idle_vcpu(snext->vcpu))? "YES":"NO");
    */
    ...
    */
    ...
    //printk("mylog - tasklet_work_scheduled : %d\n", tasklet_work_scheduled); // always 0
    ...
    */
    ret.time = (is_idle_vcpu(snext->vcpu) ?
                -1 : MILLISECS(VSCHED_MSECS_PER_TICK)); // TODO: 10 ms.. This should be checked
    ret.task = snext->vcpu;
