...
class XendDomain:
    ...
    def domain_sched_vdi_get(self, domid):
        """Get vdi scheduler parameters for a domain.

        @param domid: Domain ID or Name
        @type domid: int or string.
        @rtype: dict with keys 'weight'
        @return: vdi scheduler parameters
        """
        dominfo = self.domain_lookup_nr(domid)
        if not dominfo:
            raise XendInvalidDomain(str(domid))
        ...
        if dominfo._stateGet() in (DOM_STATE_RUNNING, DOM_STATE_PAUSED):
            try:
                return xc.sched_vdi_domain_get(dominfo.getDomid())
            except Exception, ex:
                raise XendError(str(ex))
        else:
            return {'weight' : dominfo.getWeight()}

    def domain_sched_vdi_set(self, domid, weight = None):
        """Set vdi scheduler parameters for a domain.
        ...
        try:
            ...
            rc = 0
            if dominfo._stateGet() in (DOM_STATE_RUNNING, DOM_STATE_PAUSED):
                rc = xc.sched_vdi_domain_set(dominfo.getDomid(), weight)
            if rc == 0:
                if set_weight:
                    dominfo.setWeight(weight)
                self.managed_config_save(dominfo)
