...
static PyObject *pyxc_sched_vdi_domain_set(XcObject *self,
                                              PyObject *args,
                                              PyObject *kwds)
{
    uint32_t domid;
    ...
    static char *kwd_list[] = { "domid", "weight", NULL };
    static char kwd_type[] = "I|H";
    struct xen_domctl_sched_vdi sdom;
    ...
    if ( xc_sched_vdi_domain_set(self->xc_handle, domid, &sdom) != 0 )
        return pyxc_error_to_exception(self->xc_handle);
...
static PyObject *pyxc_sched_vdi_domain_get(XcObject *self, PyObject *args)
{
    uint32_t domid;
    struct xen_domctl_sched_vdi sdom;
    ...
    if ( xc_sched_vdi_domain_get(self->xc_handle, domid, &sdom) != 0 )
        return pyxc_error_to_exception(self->xc_handle);
...
static PyMethodDef pyxc_methods[] = {
    ...
    { "sched_vdi_domain_set",
      (PyCFunction)pyxc_sched_vdi_domain_set,
      METH_KEYWORDS, "\n"
      "Set the scheduling parameters for a domain when running with the\n"
      "SMP vdi scheduler.\n"
      " domid     [int]:   domain id to set\n"
    ...
    { "sched_vdi_domain_get",
      (PyCFunction)pyxc_sched_vdi_domain_get,
      METH_VARARGS, "\n"
      "Get the scheduling parameters for a domain when running with the\n"
      "SMP vdi scheduler.\n"
      " domid     [int]:   domain id to get\n"
...
PyMODINIT_FUNC initxc(void)
{
    ...
    PyModule_AddIntConstant(m, "XEN_SCHEDULER_VDI", XEN_SCHEDULER_VDI);
