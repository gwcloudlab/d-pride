...
SUBCOMMAND_HELP = {
    ...
    'sched-vdi': ('[-d <Domain> [-w[=WEIGHT]]',
                     'Get/set vdi scheduler parameters.'),
...
def xm_sched_vdi(args):
    """Get/Set options for vdi Scheduler."""

    check_sched_type('vdi')
    ...
    if weight is None:
        ...
        for d in doms:
            try:
                if serverType == SERVER_XEN_API:
                    ...
                    info = server.xend.domain.sched_vdi_get(d['name'])
            except xmlrpclib.Fault:
                pass

            if 'weight' not in info:
                # domain does not support sched-vdi?
                info = {'weight': -1}
        ...
        if serverType == SERVER_XEN_API:
            ...
            result = server.xend.domain.sched_vdi_set(domid, weight)
            if result != 0:
                err(str(result))
