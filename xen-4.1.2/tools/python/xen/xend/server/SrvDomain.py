...
class SrvDomain(SrvDir):
    ...
    def op_domain_sched_vdi_get(self, _, req):
        fn = FormFn(self.xd.domain_sched_vdi_get,
                    [['dom', 'str']])
        val = fn(req.args, {'dom': self.dom.getName()})
        return val


    def op_domain_sched_vdi_set(self, _, req):
        fn = FormFn(self.xd.domain_sched_vdi_set,
                    [['dom', 'str'],
                     ['weight', 'int']])
        val = fn(req.args, {'dom': self.dom.getName()})
        return val
